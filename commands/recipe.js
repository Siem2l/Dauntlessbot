exports.run = (client, message, args, Discord) => {
  ...
  try{
      ...
      var recipefile = db.get("armour").find(function(o) {for (i in o["itemnames"]){if (regex.test(o['itemnames'][i])){return true;}}return false}).value();
      if(recipefile){
        armourpiece = recipefile.itemnames.findIndex(function (o){return regex.test(o)})
      ...
      if(armorarray.indexOf(args[1]) > -1 ||armorarray.indexOf(args[2]) > -1){
        console.log('t')
          if(armorarray.indexOf(args[1]) > -1 ){var armorpiece = armorarray.indexOf(args[1])}
          else if(armorarray.indexOf(args[2]) > -1){var armorpiece = armorarray.indexOf(args[2])}
          if (armorpiece){
...
regex = new RegExp( args[0]+".*", 'i');
recipefile = db.get("weapons").find(function(o) {for (i in o["itemnames"]){if (regex.test(o['itemnames'][i])){return true;}}return false}).value();
if(recipefile){
  weaponpiece = recipefile.itemnames.findIndex(function (o){return regex.test(o)})
    if (recipefile['items'][weaponpiece]['recipe_cost'].length == 0){
      return message.channel.send("This recipe is not fully known yet try again later!")
...
regex = new RegExp( args[0]+".*", 'i');
recipefile = db.get("armour").find(function(o) {for (i in o["itemnames"]){if (regex.test(o['itemnames'][i])){return true;}}return false}).value();
if(recipefile){
armourpiece = recipefile.itemnames.findIndex(function (o){return regex.test(o)})
  if (recipefile['items'][armourpiece]['recipe_cost'].length == 0){
    return message.channel.send("This recipe is not fully known yet try again later!")
